...
(function ($, window, undefined) {
	...
	$.extend($.easing, {
	 ...
	 rhinoSlider = function (element, opts) {
		...
		var
			...
			next = function ($slider, settings, $next) {
				...
				var endFlag = vars.items.last().hasClass(vars.prefix + 'active');
                /**/
				settings.callBeforeNext(endFlag);
...
function changeEffect(effectName) {
    ...
    $('#slider').rhinoslider({
        //which effect to blend content
        effect: effectName,//'none', 'fade', 'slide', 'kick', 'transfer', 'shuffle', 'explode', 'turnOver', 'chewyBars'
        //easing for animations of the slides
        easing: 'swing',
        //linear or shuffled order for items
        randomOrder: false,
        ...
        //the function, which is started before the blending-effect
        callBeforeNext: function (endFlag) {
            if (endFlag)
